...
export function resolveFeedSource(input: string, baseUrl = env.RSSHUB_BASE_URL): FeedSource {
...
export function resolveRsshubSource(uri: string, baseUrl = env.RSSHUB_BASE_URL): RsshubSource {
  ...
  const fetchUrl = new URL(route, ensureTrailingSlash(baseUrl)).toString();
