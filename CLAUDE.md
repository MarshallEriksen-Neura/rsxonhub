...
- **State management:** Zustand for client-side state. When data needs to flow across multiple component layers (deep prop drilling), put it in a Zustand store under `lib/stores/` instead of passing props through many levels. Use `persist` middleware from `zustand/middleware` only for non-secret local preferences such as theme. Do not store AI API keys in client storage; AI runtime config is DB-backed from `/settings`.
...
- **AI access:** business code must go through the `lib/ai/` factory, never a provider SDK directly. Chat and embedding are configured separately (different endpoints/models). Both are OpenAI-compatible via DB-backed `baseURL` + `apiKey` rows in `ai_configs`; runtime calls must use the retry wrapper for transient provider failures.
...
- **pgvector dimension is fixed at table-creation time.** The runtime embedding endpoint/model is DB-configurable, but `article_chunks.embedding vector(...)` is locked by the source-level `DEFAULT_EMBEDDING_DIM` constant and its generated migration. If the embedding model dimension changes, create a migration/re-embedding plan instead of changing settings only.
