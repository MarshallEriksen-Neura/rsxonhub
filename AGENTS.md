...
<!-- PROJECT CONVENTIONS (rsxonhub) — DO NOT REMOVE -->
## UI: RetroUI first — do not reinvent components
The UI framework is **RetroUI**, installed via the shadcn registry `@retroui` (`https://retroui.dev/r/{name}.json`); components live in `components/retroui/`. When you need a UI component:
1. Check `components/retroui/` and reuse an existing one.
